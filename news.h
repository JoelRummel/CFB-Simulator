	...
	struct Story {
		...
		Story(Topic t, double prio, std::vector<School*> s = {}, std::vector<Player*> p = {}, std::vector<std::string> i = {}) {
			...
			case Topic::RIVALRY_MATCHUP:
				...
				if (s[0]->getName() == "Auburn" || s[0]->getName() == "Alabama")
					headerOptions = { "Auburn and Alabama look to settle the score", "Foy-ODK trophy up for grabs",
									  "'Iron Bowl' competitors hungry for blood" };
				break;
			...
			case Topic::FINALS_SET:
				priority = 25;
				headerOptions = { "Championship game set", "Playoff finals are here", "Playoff finalists face off this week" };
			...
			case Topic::BIG_MATCHUP:
				...
				priority = 15;
				headerOptions = { "Two titans clash", "Highly anticipated showdown awaits", "Hype builds for big game", "Bragging rights await" };
				bodyOptions = { "Regular season game between " + sn[0] + " and " + sn[1] + " looks sure to excite many fans",
								"Playoff positioning is on the line as " + sn[0] + " and " + sn[1] + " clash this weekend",
								"Two confident teams in " + sn[0] + " and " + sn[1] + " head for a collision, look to hang on to playoff hopes" };
				break;

			case Topic::CONFERENCE_OPENER:
				priority = 4;
				headerOptions = { sn[0] + " begins conference play", sn[0] + " to make conference debut" };
			...
			case Topic::SEASON_OPENER:
				priority = 3;
				headerOptions = { "Excitement builds for " + sn[0], sn[0] + " begins a hopeful season", sn[0] + " expectations soar" };
			...
			case Topic::RANKED_G5_MATCHUP:
				priority = 14;
				headerOptions = { sn[0] + " seeks respect", "Big test coming for " + sn[0] };
			...
			case Topic::STANDARD_MATCHUP:
				priority = 2;
				headerOptions = { sn[1] + " dreams of an upset", sn[0] + " feeling confident", sn[1] + " takes aim at " + sn[0] };
				bodyOptions = { sn[1] + " has nothing to lose and everything to gain as they prepare to face a daunting opponent in " + sn[0],
			...
			case Topic::CONFERENCE_CHAMPIONS:
				priority = 20;
				headerOptions = { i[0] + " champion is crowned", sn[0] + " wins the " + i[0], sn[0] + " are " + i[0] + " champions" };
